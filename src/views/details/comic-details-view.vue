...
export default {
  ...
  mounted() {
    getById("comics", this.id).then((response) => {
      ...
      this.imgComicStyle = `background-image: url(${path}.${extension}); background-repeat: no-repeat; background-size: cover;`;
      this.loading = false;

      // We recover the comics where the character appeared
      const { collectionURI } = characters;
